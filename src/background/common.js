...
let group_id = "";
export function reset_page(tabId) {
    ...
    chrome.webRequest.onBeforeSendHeaders.addListener(
      function (details) {
        openDB().then(async (db) => {
          if (details.url.indexOf("convert-group/status") !== -1) {
            group_id = new URL(details.url).searchParams.get("id");
            await set(details.tabId + "-kdoc_download_info_group_id", group_id, db);
            // 设置Header
            const headers = {
            ...
            const fileIdsArray = fileIdsParam ? fileIdsParam.split(",") : [];
            const current_tab_info = await readData(details.tabId + "-kdoc_download_info", db)
            if (current_tab_info === undefined) {
              await cleanTabDownloadPath(details.tabId + "-kdoc_download_info");
            }
            const new_download_path_list = [...new Set([...current_tab_info.value, ...fileIdsArray])];
            await set(details.tabId + "-kdoc_download_info", new_download_path_list, db);
          }
        });
      },
      {
...
function extension_check(details) {
...
export async function addDownloadPathData(tabId, data, db) {
  return new Promise(async (resolve, reject) => {
    if (db === undefined) {
      db = await getDb();
    }
    ...
    getRequest.onsuccess = function () {
      let _data = getRequest.result ? getRequest.result.pathList : [];
      ...
      // 然后立即更新数据
      objectStore.put({ id: tabId, pathList: _data });
    };

    getRequest.onerror = function () {
      // 如果读取失败，则尝试添加新数据
      objectStore.put({ id: tabId, pathList: [data] });
    };
  });
}
...
export async function removeAllData(db) {
  return new Promise(async (resolve, reject) => {
    ...
    let transaction = db.transaction(["AsyncFetcherObjectStore"], "readwrite");
    let objectStore = transaction.objectStore("AsyncFetcherObjectStore");
    let request = objectStore.clear(); // 使用 delete 方法删除指定 key 的记录
...
export async function cleanDataDownloadPath(col_name, db) {
  return new Promise(async (resolve, reject) => {
    if (db === undefined) {
      db = await getDb();
    }
    ...
    // 先尝试从数据库中读取数据
    let getRequest = objectStore.get(col_name);

    getRequest.onsuccess = function () {
      // 清空接口数据
      objectStore.put({ id: col_name, pathList: [] });
    };
...
export async function cleanTabDownloadPath(col_name, db) {
  return new Promise(async (resolve, reject) => {
    if (db === undefined) {
      db = await getDb();
    }
    ...
    getRequest.onsuccess = function () {
      // 清空接口数据
      objectStore.put({ id: col_name, value: [] });
    };
